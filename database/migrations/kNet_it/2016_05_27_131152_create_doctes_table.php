...
class CreateDoctesTable extends Migration
{
    ...
    public function up()
    {
        Schema::connection(env('DB_CNCT_IT', 'kNet_it'))->create('doctes', function (Blueprint $table) {
            $table->bigInteger('id')->unsigned()->primary()->comment('ID univoco per ogni documento');
            ...
            $table->boolean('u_rnc')->nullable()->comment('Presenza Non Conformità');
            $table->integer('u_numrnc')->unsigned()->nullable()->comment('Numero Documento RNC');
            $table->bigInteger('u_idrnc')->unsigned()->nullable()->comment('ID Riferimento Tabella RNC');
    ...
    {
        Schema::connection(env('DB_CNCT_IT', 'kNet_it'))->drop('doctes');
    }
}
